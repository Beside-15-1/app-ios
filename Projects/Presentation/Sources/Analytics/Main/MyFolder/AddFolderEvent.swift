...
enum AddFolderEvent {
  case click(component: AddFolderClickComponent)
  case shown
}

// MARK: PBAnalyticsType

extension AddFolderEvent: PBAnalyticsType {
  var name: String {
    switch self {
    case .click:
      return PBAnalyticsEventNameBuilder()
        .action(with: .clicked)
        .screen(with: "addFolder")
        .version(with: 1)
        .build()
...
enum AddFolderClickComponent: String {
  case folderTitleTab
  case folderColorTab
